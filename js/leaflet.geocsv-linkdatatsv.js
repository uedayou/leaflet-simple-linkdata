...
L.GeoCSV = L.GeoJSON.extend({
  ...
  options: {
    ...
    // LinkData 用設定
    useLinkData: true,
    labelLatitude: "http://www.w3.org/2003/01/geo/wgs84_pos#lat",
    labelLongitude: "http://www.w3.org/2003/01/geo/wgs84_pos#long",
  },
...
//        , lat = parseFloat(campos[titulos.indexOf('lat')]);
        , lng = parseFloat(campos[titulos.indexOf(this.options.labelLongitude)])
        , lat = parseFloat(campos[titulos.indexOf(this.options.labelLatitude)]);
//      if (campos.length==titulos.length && lng<180 && lng>-180 && lat<90 && lat>-90) {
      if (campos.length<=titulos.length && lng<180 && lng>-180 && lat<90 && lat>-90) {
        ...
        for (var i=0; i<campos.length; i++) {
          if (titulos[i] != this.options.labelLatitude && titulos[i] != this.options.labelLongitude) {
